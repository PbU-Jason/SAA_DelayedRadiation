...
filename = 'MASS_SAA_5700s_smaller20.pickle'
...
for i in range(0,len(counts_bin),1):
    flux_bin.append((counts_bin[i]*1000*20)/(Area*float(4700)*(bins[i+1]-bins[i]))) #!!! (# in OneBin) to (#/(MeV*cm^2*s))
...
for file_index in range(len(file_list)):
    ...
    while line:
        ...
        if len(line_list) != 0: # skip empty results
            ...
            if int(line_list[0][0]) == 3: # CZT HTsim
                event_list_temp.append(line_list)
            else: # 4, so it is ACS HTsim
                ACS_hit += 1
